...
namespace game
{
    ...
    void killed(fpsent *d, fpsent *actor)
    {
        ...
        {
	    if(d==player1) conoutf(contype, "\f2%s got fragged by %s with %s", dname, aname, guns[actor->gunselect].name);
	    else conoutf(contype, "\f2%s fragged %s with %s", aname, dname, guns[actor->gunselect].name);
        }
        deathstate(d);
		ai::killed(d, actor);
    }
