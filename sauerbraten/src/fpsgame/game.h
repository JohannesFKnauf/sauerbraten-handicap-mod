...
} gamemodes[] =
{
    ...
    { "handicap", M_HANDICAP, "Handicap: Frag everyone to increase your handicap. Handicap corresponds to frag differences between players. Health, armor, damage are reduced by current handicap." },
...
#define m_handicap     (m_check(gamemode, M_HANDICAP))
...
#define PROTOCOL_VERSION 532            // symbolic high value to force incompatibility of handicap with vanilla sauerbraten
...
{
    { S_PUNCH1,    250, 100,   0,   0,  0,   14,  1,  80,  0,    0, "fist",            "fist",   0 },
    ...
    { S_FLAUNCH,   500,  90,   0, 200, 10, 1024,  1, 250, 45, 1500, "grenadelauncher", "gl",     0 },
...
struct fpsstate
{
    ...
    fpsstate() : maxhealth(100), handicap(100), aitype(AI_NONE), skill(0) {}
    ...
    bool canpickup(int type)
    {
        ...
        switch(type)
        {
            ...
            case I_HEALTH: return health < maxhealth * handicap / 100;
            case I_GREENARMOUR:
                // (100h/100g only absorbs 200 damage)
                if(armourtype==A_YELLOW && armour>=100) return false;
            case I_YELLOWARMOUR: return !armourtype || armour<(is.max * handicap / 100);
    ...
    void pickup(int type)
    {
        ...
        switch(type)
        {
            ...
            case I_HEALTH: // boost also adds to health
                health = min(health+is.add, maxhealth * handicap / 100);
                break;
            case I_GREENARMOUR:
            case I_YELLOWARMOUR:
                armour = min(armour+is.add, (is.max * handicap / 100));
                armourtype = is.info;
                break;
            case I_QUAD:
