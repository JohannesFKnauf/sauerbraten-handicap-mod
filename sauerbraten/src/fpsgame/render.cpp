...
{      
    ...
    void rendergame(bool mainpass)
    {
	    ...
	    if(m_handicap) { defformatstring(sn)(" @ %d", d->handicap); concatstring(d->info, sn); }
            if(d->state!=CS_DEAD) particle_text(d->abovehead(), d->info, PART_TEXT, 1, team ? (team==1 ? 0x6496FF : 0xFF4B19) : 0x1EC850, 4.0f);
        }
        loopv(ragdolls)
        {
            fpsent *d = ragdolls[i];
