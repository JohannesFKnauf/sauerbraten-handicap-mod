...
namespace game
{
    ...
    void updatebouncers(int time)
    {
        loopv(bouncers)
        {
            ...
            if(stopped)
            {
                ...
                {
		    int qdam = guns[GUN_GL].damage*(bnc.owner->quadmillis ? 4 : 1) * bnc.owner->handicap / 100;
                    hits.setsize(0);
    ...
    void updateprojectiles(int time)
    {
        loopv(projs)
        {
            ...
            int qdam = guns[p.gun].damage*(p.owner->quadmillis ? 4 : 1) * p.owner->handicap / 100;
	...
	d->totalshots += guns[d->gunselect].damage*(d->quadmillis ? 4 : 1)*guns[d->gunselect].rays * d->handicap / 100;
    }
